-- Migration: fix_run_payroll_final
-- หน้าที่: lock version สุดท้ายของ run_payroll — map calculation_method, status fix, delete on fatal
-- Date: 29 เม.ย. 2569

-- Drop unique constraint (ถ้ายังมีอยู่)
ALTER TABLE payroll_runs 
DROP CONSTRAINT IF EXISTS payroll_runs_period_year_period_month_round_key;
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF (v_calc->>'success')::boolean THEN
        ...
        v_calc_method := CASE v_calc->>'mode'
          WHEN 'new_hire'    THEN 'days_worked'
          WHEN 'resignation' THEN 'days_worked'
          WHEN 'same_month'  THEN 'days_worked'
          ELSE 'days_absent'
        END;
        ...
        INSERT INTO payroll_details (
          payroll_run_id, employee_id, calculation_method,
          base_salary, worked_days, absent_days,
          prorated_salary, ot_amount, diligence_bonus, diligence_forfeited,
          gross_income, sso_base, sso_employee, sso_employer,
          wht_amount, net_pay
        ) VALUES (
          v_run_id, v_emp.id, v_calc_method,
      ...
      ELSE
        v_errors := v_errors || jsonb_build_object('employee_id', v_emp.id, 'error', v_calc->>'error');
        v_error_count := v_error_count + 1;
      END IF;

    EXCEPTION WHEN OTHERS THEN
      v_errors := v_errors || jsonb_build_object('employee_id', v_emp.id, 'error', SQLERRM);
      v_error_count := v_error_count + 1;
    END;
  END LOOP;
  ...
  UPDATE payroll_runs SET
    ...
    status             = CASE WHEN v_error_count = 0 THEN 'approved' ELSE 'draft' END,
    updated_at         = NOW()
  WHERE id = v_run_id;
