-- Fix: calculate_payroll — align กับ schema จริง
-- Date: 29 เม.ย. 2569
...
BEGIN
  ...
  IF v_emp.employment_type = 'pc' OR v_emp.role = 'sso_only' THEN
  ...
  -- resign date = deleted_at ถ้า status = resigned
  v_resign_date := CASE
  ...
  SELECT COALESCE(SUM(
    CASE
      ...
      ELSE
        ROUND((v_base_salary / 30.0 / 8.0) * ot.rate_multiplier * ot.ot_hours, 2)
    END
  ), 0)
  INTO v_ot_amount
  FROM ot_requests ot
  ...
  -- diligence amount ดึงจาก cost_centers หรือ hardcode ตาม MRD
  -- ตอนนี้ใช้ 500 เป็น default — ปรับเป็น DB-driven ก่อน go-live
  ...
  -- 8. SSO (D10)
  -- ────────────────────────────────────────────
  -- สังวาลย์ = sso_only → ไม่เข้า payroll แต่ถ้าใครมี sso_id → คำนวณปกติ
  IF v_emp.employment_status != 'resigned' OR v_resign_date >= v_month_start THEN
  ...
  -- ────────────────────────────────────────────
  -- 9. WHT — annualize จาก full salary
  -- ────────────────────────────────────────────
